            ...
            if (Physics.Raycast(ray, out raycastHit))
            {
                if (raycastHit.transform.gameObject == transform.gameObject & Vector3.Distance(this.player.position, base.transform.position) < 10f & Cursor.lockState == CursorLockMode.Locked)
                {
                    if (transform.name == "Pickup_Slab")
                    {
                        ...
                        {
                            if (((ID == 25 && gc.item[gc.itemSelected] == 25) || (ID != 25)) && GetComponentInChildren<SpriteRenderer>().sprite.texture != gc.itemTextures[0] && gc.item[gc.itemSelected] != 0)
                            {
                                gc.CurseOfRa();
                        ...
                        else
                        {
                            ...
                            if (ID == 25 && gc.item[gc.itemSelected] != 0)
                            {
                                gc.UndoCurse();
                            }
                            else if (ID != 25)
